...
export function OxSubmitButton({ symbol }: OxSubmitButtonProps) {
  ...
  return (
    <button
      ...
      disabled={pending}
      className={clsx(
        "flex h-32 w-full flex-col items-center justify-center gap-2 border border-line transition-all duration-150",
    ...
    >
      <span className="text-5xl font-light">{symbol}</span>
    </button>
  );
}
