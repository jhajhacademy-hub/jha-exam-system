...
export async function addRetakeNotificationEmailAction(
  _prevState: NotificationFormState,
  formData: FormData
): Promise<NotificationFormState> {
  await requireAdminProfile();
  ...
  const { data: settings, error: fetchError } = await admin
    .from("site_settings")
    .select("retake_notification_emails")
    .eq("id", 1)
    .single();
  if (fetchError) {
    return { status: "error", message: `取得に失敗しました: ${fetchError.message}` };
  ...
  const current = settings?.retake_notification_emails ?? [];
  if (current.includes(email)) {
    return { status: "error", message: "そのメールアドレスは既に登録されています。" };
  }
  ...
  const { error: updateError } = await admin
    .from("site_settings")
    .update({
      retake_notification_emails: [...current, email],
      updated_at: new Date().toISOString(),
    })
    .eq("id", 1);
  if (updateError) {
...
export async function removeRetakeNotificationEmailAction(formData: FormData) {
  await requireAdminProfile();
  ...
  await admin
    .from("site_settings")
    .update({
      retake_notification_emails: current.filter((e) => e !== email),
      updated_at: new Date().toISOString(),
    })
    .eq("id", 1);
